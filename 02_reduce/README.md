...
- 静态内存代替动态分配内存
    在之前的包装函数reduce中，需要为数组d_y分配与释放device内存，实际上这是比较耗时的，可以用静态全局内存代替这里的动态全局内存，在编译期间分配，避免在运行时反复分配，利用cudaGetSymbolAddress 将该指针和全局变量static_y联系起来